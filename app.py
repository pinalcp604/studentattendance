...
# Columns: Student ID, Password
students_df = pd.read_excel('students.xlsx', engine='openpyxl')
students_df['Student ID'] = students_df['Student ID'].astype(
    str)  # Ensure Student ID is string

# Columns: Date, Student ID, Check-In Time, Check-Out Time
attendance_df = pd.read_excel('attendance.xlsx', engine='openpyxl')
attendance_df['Student ID'] = attendance_df['Student ID'].astype(
    str)  # Ensure Student ID is string
...
# def authenticate(student_id, password):
   # student = students_df[(
  #      students_df['Student ID'].str.strip() == student_id)]
